glslangValidator.exe -V CubeMapFragmentShader.frag -o 		CubeMapFragmentShader.spv
glslangValidator.exe -V CubeMapVertexShader.vert -o 		CubeMapVertexShader.spv
glslangValidator.exe -V LightingFragmentShader.frag -o 		LightingFragmentShader.spv
glslangValidator.exe -V SceneBufferFragmentShader.frag -o 	SceneBufferFragmentShader.spv
glslangValidator.exe -V SceneBufferVertexShader.vert -o 	SceneBufferVertexShader.spv
glslangValidator.exe -V ViewportVertexShader.vert -o 		ViewportVertexShader.spv
pause