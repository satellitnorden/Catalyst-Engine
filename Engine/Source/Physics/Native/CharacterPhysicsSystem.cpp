...
*/
void CharacterPhysicsSystem::UpdateCharacterMovement(CharacterMovement *const RESTRICT movement) NOEXCEPT
{
	//Simulate physics on the character movement.
	SimulatePhysicsCharacterMovement(movement);
...
*/
void CharacterPhysicsSystem::SimulatePhysicsCharacterMovement(CharacterMovement *const RESTRICT movement) NOEXCEPT
{
	...
	{
		//Apply gravity.
		movement->_Velocity._Y -= PhysicsConstants::GRAVITY * delta_time;

		//Apply the velocity to the position.
		movement->_Position += movement->_Velocity * delta_time;
	}
	...
	else
	{
		...
		movement->_Position += movement->_MovementInput * delta_time;
...
NO_DISCARD bool CharacterPhysicsSystem::CheckCollisionCharacterMovement(const CharacterMovement *const RESTRICT movement, Vector3<float32> *const RESTRICT collision_position) NOEXCEPT
{
	...
	*/
	...
	//Clamp collision to zero height.
	if (movement->_Position._Y < 0.0f)
	{
		//Collision was found!
		*collision_position = Vector3<float32>(movement->_Position._X, 0.0f, movement->_Position._Z);
