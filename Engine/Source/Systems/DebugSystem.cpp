...
*/
void DebugSystem::RegisterCustomDebugCommand
(
...
) NOEXCEPT
{
	...
	for (;;)
	{
		...
		else
		{
			...
			new_debug_command._Type = DebugCommand::Type::CUSTOM;
		...
		for (DebugCommand &debug_command : debug_category._DebugCommands)
		{
			...
			switch (debug_command._Type)
			{
				case DebugCommand::Type::CUSTOM:
				{
					...
					if (ImGui::CollapsingHeader(debug_command._Name.Data()))
					{
						debug_command._State._CustomState._Depth = depth + 2;
