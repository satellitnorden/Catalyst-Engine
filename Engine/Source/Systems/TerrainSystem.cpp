		...
		if (TerrainQuadTreeUtilities::ShouldBeSubdivided(component, *node, camera_position))
		{
			...
			for (uint64 current_node_index{ 0 }; current_node_index < 4; ++current_node_index)
			{
				...
				*/
				...
				child_node._MinimumHeightMapCoordinate = (child_node._Minimum + (Vector2<float32>(static_cast<float32>(component->_PatchSize))) * 0.5f) / static_cast<float32>(component->_PatchSize);
				child_node._MaximumHeightMapCoordinate = (child_node._Maximum + (Vector2<float32>(static_cast<float32>(component->_PatchSize))) * 0.5f) / static_cast<float32>(component->_PatchSize);
