...
{
	PROFILING_SCOPE("AnimatedModelAssetCompiler::Compile");
...
*/
void AnimatedModelAssetCompiler::Load(const LoadContext &load_context) NOEXCEPT
{
	PROFILING_SCOPE("AnimatedModelAssetCompiler::Load");

	//Allocate the asset.
	AnimatedModelAsset *const RESTRICT new_asset{ static_cast<AnimatedModelAsset *const RESTRICT>(load_context._Asset) };
	...
	for (uint64 mesh_index{ 0 }; mesh_index < number_of_meshes; ++mesh_index)
	{
		...
		load_context._StreamArchive->Read(vertices[mesh_index].Data(), sizeof(AnimatedVertex) * number_of_vertices, &stream_archive_position);
		...
		load_context._StreamArchive->Read(indices[mesh_index].Data(), sizeof(uint32) * number_of_indices, &stream_archive_position);
	}
	...
	//Create the buffers.
	{
		...
		RenderingSystem::Instance->CreateBuffer(data_sizes[0], BufferUsage::StorageBuffer | BufferUsage::VertexBuffer, MemoryProperty::DeviceLocal, &new_asset->_VertexBuffer);
		RenderingSystem::Instance->UploadDataToBuffer(data_chunks, data_sizes, 1, &new_asset->_VertexBuffer);
	}

	{
		const void *const RESTRICT data_chunks[]{ indices[0].Data() };
		const uint64 data_sizes[]{ sizeof(uint32) * indices[0].Size() };
		RenderingSystem::Instance->CreateBuffer(data_sizes[0], BufferUsage::IndexBuffer | BufferUsage::StorageBuffer, MemoryProperty::DeviceLocal, &new_asset->_IndexBuffer);
		RenderingSystem::Instance->UploadDataToBuffer(data_chunks, data_sizes, 1, &new_asset->_IndexBuffer);
	}

	//Write the index count.
	new_asset->_IndexCount = static_cast<uint32>(indices[0].Size());

	//Read the parent transform.
	load_context._StreamArchive->Read(&new_asset->_ParentTransform, sizeof(Matrix4x4), &stream_archive_position);

	//Read the bones.
	ReadBone(&new_asset->_Skeleton._RootBone, load_context._StreamArchive, &stream_archive_position);

	//Read the total number of bones.
	load_context._StreamArchive->Read(&new_asset->_Skeleton._TotalNumberOfBones, sizeof(uint32), &stream_archive_position);
	...
	//Update the total CPU/GPU memory.
	{
		...
		cpu_memory += sizeof(AnimatedModelAsset::_IndexCount);
		cpu_memory += CalculateBoneMemoryUsage(new_asset->_Skeleton._RootBone);
