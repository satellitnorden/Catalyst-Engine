...
{
	PROFILING_SCOPE("LevelAssetCompiler::Compile");
...
*/
void LevelAssetCompiler::Load(const LoadContext &load_context) NOEXCEPT
{
	PROFILING_SCOPE("LevelAssetCompiler::Load");

	//Allocate the asset.
	LevelAsset *const RESTRICT new_asset{ static_cast<LevelAsset *const RESTRICT>(load_context._Asset) };

	//Read the data.
	uint64 stream_archive_position{ load_context._StreamArchivePosition };

	//Read the level statistics.
	load_context._StreamArchive->Read(&new_asset->_LevelStatistics, sizeof(LevelStatistics), &stream_archive_position);
