...
DEFINE_SINGLETON(HighDetailTreeVegetationCrownColorRenderPass);
...
*	Default constructor.
*/
HighDetailTreeVegetationCrownColorRenderPass::HighDetailTreeVegetationCrownColorRenderPass() NOEXCEPT
{
	//Set the initialization function.
	SetInitializationFunction([](void *const RESTRICT)
	{
		HighDetailTreeVegetationCrownColorRenderPass::Instance->InitializeInternal();
	});
}
...
*/
void HighDetailTreeVegetationCrownColorRenderPass::InitializeInternal() NOEXCEPT
{
	//Set the main stage.
	SetMainStage(RenderPassMainStage::Scene);

	//Set the sub stage.
	SetSubStage(RenderPassSubStage::HighDetailTreeVegetationCrownColor);

	//Set the shaders.
	SetVertexShader(Shader::HighDetailTreeVegetationCrownColorVertex);
	SetTessellationControlShader(Shader::None);
	SetTessellationEvaluationShader(Shader::None);
	SetGeometryShader(Shader::None);
	SetFragmentShader(Shader::HighDetailTreeVegetationCrownColorFragment);
	...
	{
		HighDetailTreeVegetationCrownColorRenderPass::Instance->RenderInternal();
	});
...
/*
*	Renders the crown color of the high detail tree vegetation.
*/
void HighDetailTreeVegetationCrownColorRenderPass::RenderInternal() NOEXCEPT
{
	...
	for (const TreeVegetationTypeInformation &information : *informations)
	{
		...
		for (const TreeVegetationPatchRenderInformation &renderInformation : information._PatchRenderInformations)
		{
			...
			if (!TEST_BIT(renderInformation._Visibilities[UNDERLYING(TreeVegetationLevelOfDetail::High)], VisibilityFlag::Viewer)
			...
			commandBuffer->BindVertexBuffer(this, 1, renderInformation._TransformationsBuffers[UNDERLYING(TreeVegetationLevelOfDetail::High)], &offset);
