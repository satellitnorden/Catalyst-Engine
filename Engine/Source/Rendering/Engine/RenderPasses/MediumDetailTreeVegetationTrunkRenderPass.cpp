...
DEFINE_SINGLETON(MediumDetailTreeVegetationTrunkRenderPass);
...
*	Default constructor.
*/
MediumDetailTreeVegetationTrunkRenderPass::MediumDetailTreeVegetationTrunkRenderPass() NOEXCEPT
{
	//Set the initialization function.
	SetInitializationFunction([](void *const RESTRICT)
	{
		MediumDetailTreeVegetationTrunkRenderPass::Instance->InitializeInternal();
	});
}
...
*/
void MediumDetailTreeVegetationTrunkRenderPass::InitializeInternal() NOEXCEPT
{
	//Set the main stage.
	SetMainStage(RenderPassMainStage::Scene);

	//Set the sub stage.
	SetSubStage(RenderPassSubStage::MediumDetailTreeVegetationTrunk);

	//Set the shaders.
	SetVertexShader(Shader::MediumDetailTreeVegetationTrunkVertex);
	SetTessellationControlShader(Shader::None);
	SetTessellationEvaluationShader(Shader::None);
	SetGeometryShader(Shader::None);
	SetFragmentShader(Shader::MediumDetailTreeVegetationTrunkFragment);
	...
	{
		MediumDetailTreeVegetationTrunkRenderPass::Instance->RenderInternal();
	});
...
*/
void MediumDetailTreeVegetationTrunkRenderPass::RenderInternal() NOEXCEPT
{
	...
	for (const TreeVegetationTypeInformation &information : *informations)
	{
		...
		commandBuffer->BindVertexBuffer(this, 0, information._Model._TrunkBuffers[UNDERLYING(LevelOfDetail::Medium)], &offset);
		commandBuffer->BindIndexBuffer(this, information._Model._TrunkBuffers[UNDERLYING(LevelOfDetail::Medium)], information._Model._TrunkIndexOffsets[UNDERLYING(LevelOfDetail::Medium)]);
		...
		for (const TreeVegetationPatchRenderInformation &renderInformation : information._PatchRenderInformations)
		{
			...
			//Draw the instances!
			commandBuffer->DrawIndexed(this, information._Model._TrunkIndexCounts[UNDERLYING(LevelOfDetail::Medium)], renderInformation._NumberOfTransformations[UNDERLYING(TreeVegetationLevelOfDetail::Medium)]);
		}
	}
