...
*/
void DiffuseIrradianceRayTracingPipeline::Initialize() NOEXCEPT
{
	...
	SetRayGenerationShader(Shader::DiffuseIrradianceRayGeneration);

	//Set the closest hit shader.
	SetClosestHitShader(Shader::DiffuseIrradianceRayClosestHit);

	//Add the miss shaders.
	SetNumberOfMissShaders(2);
	AddMissShader(Shader::DiffuseIrradianceRayMiss);
...
*/
void DiffuseIrradianceRayTracingPipeline::Execute() NOEXCEPT
{
	...
	RenderingSystem::Instance->BindCombinedImageSamplerToRenderDataTable(2, 0, &_RenderDataTable, ResourceLoader::GetTextureCube(HashString("Environment_TextureCube")), RenderingSystem::Instance->GetSampler(Sampler::FilterLinear_MipmapModeNearest_AddressModeClampToEdge));
...
*/
void DiffuseIrradianceRayTracingPipeline::CreateRenderDataTableLayout() NOEXCEPT
{
...
*/
void DiffuseIrradianceRayTracingPipeline::CreateRenderDataTable() NOEXCEPT
{
	...
	RenderingSystem::Instance->BindStorageImageToRenderDataTable(1, 0, &_RenderDataTable, RenderingSystem::Instance->GetRenderTarget(RenderTarget::DiffuseIrradiance));
}