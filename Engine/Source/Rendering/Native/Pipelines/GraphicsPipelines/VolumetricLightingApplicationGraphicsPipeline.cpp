...
void VolumetricLightingApplicationGraphicsPipeline::Initialize(const RenderTargetHandle source) NOEXCEPT
{
	...
	//Set the name.
	SetName("Volumetric Lighting Application");
#endif
}
...
void VolumetricLightingApplicationGraphicsPipeline::CreateRenderDataTable(const RenderTargetHandle source) NOEXCEPT
{
	...
	RenderingSystem::Instance->BindCombinedImageSamplerToRenderDataTable(2, 0, &_RenderDataTable, PostSceneFeaturesRenderPass::Instance->GetDepthMip(0), RenderingSystem::Instance->GetSampler(Sampler::FilterNearest_MipmapModeNearest_AddressModeClampToEdge));
}