																	...
																	const RenderTargetHandle intermediate_ambient_occlusion_render_target) NOEXCEPT
{
	...
	SetFragmentShader(ResourceSystem::Instance->GetShaderResource(HashString("AmbientOcclusionTemporalDenoisingFragmentShader"))->_Handle);
...
void AmbientOcclusionTemporalDenoisingGraphicsPipeline::CreateRenderDataTable(const RenderTargetHandle previous_temporal_buffer_render_target, const RenderTargetHandle ambient_occlusion_render_target) NOEXCEPT
{
	...
	RenderingSystem::Instance->BindCombinedImageSamplerToRenderDataTable(2, 0, &_RenderDataTable, RenderingSystem::Instance->GetRenderTarget(RenderTarget::SCENE_FEATURES_4_HALF), RenderingSystem::Instance->GetSampler(Sampler::FilterLinear_MipmapModeNearest_AddressModeClampToEdge));
}