...
void VulkanRenderingSystem::InitializeVulkanRenderPasses() NOEXCEPT
{
	...
	{
		constexpr uint64 NUMBER_OF_SUBPASSES{ 29 };
		...
		StaticArray<VkSubpassDependency, NUMBER_OF_SUBPASSES - 1> subpassDependencies
		{
			...
			VulkanUtilities::CreateSubpassDependency(	VulkanTranslationUtilities::GetSubStageIndex(RenderPassMainStage::Scene, RenderPassSubStage::LowDetailGrassVegetationColor),
														VulkanTranslationUtilities::GetSubStageIndex(RenderPassMainStage::Scene, RenderPassSubStage::ParticleSystem),
														VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT,
														VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT,
														VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT,
														VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT,
	...
	//Initialize the exponential height fog render pass.
	{
		...
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::ExponentialHeightFog)]._RenderPass = VulkanInterface::Instance->CreateRenderPass(renderPassParameters);

		//Create the framebuffer.
		VulkanFramebufferCreationParameters framebufferParameters;

		framebufferParameters._RenderPass = _VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::ExponentialHeightFog)]._RenderPass->Get();
		...
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::ExponentialHeightFog)]._FrameBuffers.Reserve(1);
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::ExponentialHeightFog)]._FrameBuffers.EmplaceFast(VulkanInterface::Instance->CreateFramebuffer(framebufferParameters));
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::ExponentialHeightFog)]._NumberOfAttachments = 1;
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::ExponentialHeightFog)]._ShouldClear = false;
	}
