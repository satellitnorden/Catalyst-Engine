...
void VulkanRenderingSystem::InitializeVulkanRenderPasses() NOEXCEPT
{
	...
	//Initialize the screen space ambient occlusion down sample first iteration render pass.
	{
		constexpr uint64 SUBPASSES{ 1 };
	...
	//Initialize the bloom downsample first iteration render pass.
	{
		...
		constexpr uint32 SCENE_INTERMEDIATE_HALF_INDEX{ 0 };
		...
		{
			//Screen.
			VulkanUtilities::CreateAttachmentDescription(	static_cast<VulkanRenderTarget *const RESTRICT>(RenderingSystem::Instance->GetRenderTarget(RenderTarget::IntermediateHalf))->GetFormat(),
															VK_ATTACHMENT_LOAD_OP_DONT_CARE,
															VK_ATTACHMENT_STORE_OP_STORE,
															VK_ATTACHMENT_LOAD_OP_DONT_CARE,
															VK_ATTACHMENT_STORE_OP_DONT_CARE,
		...
		constexpr StaticArray<const VkAttachmentReference, 1> colorAttachmentReferences
		{
			VkAttachmentReference{ SCENE_INTERMEDIATE_HALF_INDEX, VK_IMAGE_LAYOUT_GENERAL }
		};
		...
		StaticArray<VkImageView, 1> attachments
		{
			static_cast<VulkanRenderTarget *const RESTRICT>(RenderingSystem::Instance->GetRenderTarget(RenderTarget::IntermediateHalf))->GetImageView()
		};
	...
	//Initialize the bloom downsample second iteration render pass.
	{
		constexpr uint64 SUBPASSES{ 1 };

		constexpr uint32 SCENE_INTERMEDIATE_QUARTER_INDEX{ 0 };
		...
		{
			//Screen.
			VulkanUtilities::CreateAttachmentDescription(	static_cast<VulkanRenderTarget *const RESTRICT>(RenderingSystem::Instance->GetRenderTarget(RenderTarget::IntermediateQuarter))->GetFormat(),
															VK_ATTACHMENT_LOAD_OP_DONT_CARE,
															VK_ATTACHMENT_STORE_OP_STORE,
															VK_ATTACHMENT_LOAD_OP_DONT_CARE,
															VK_ATTACHMENT_STORE_OP_DONT_CARE,
		...
		constexpr StaticArray<const VkAttachmentReference, 1> colorAttachmentReferences
		{
			VkAttachmentReference{ SCENE_INTERMEDIATE_QUARTER_INDEX, VK_IMAGE_LAYOUT_GENERAL }
		};
		...
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleSecondIteration)]._RenderPass = VulkanInterface::Instance->CreateRenderPass(renderPassParameters);

		//Create the framebuffer.
		VulkanFramebufferCreationParameters framebufferParameters;

		framebufferParameters._RenderPass = _VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleSecondIteration)]._RenderPass->Get();

		StaticArray<VkImageView, 1> attachments
		{
			static_cast<VulkanRenderTarget *const RESTRICT>(RenderingSystem::Instance->GetRenderTarget(RenderTarget::IntermediateQuarter))->GetImageView()
		};
		...
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleSecondIteration)]._FrameBuffers.Reserve(1);
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleSecondIteration)]._FrameBuffers.EmplaceFast(VulkanInterface::Instance->CreateFramebuffer(framebufferParameters));
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleSecondIteration)]._NumberOfAttachments = 1;
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleSecondIteration)]._ShouldClear = false;
	}

	//Initialize the bloom downsample third iteration render pass.
	{
		constexpr uint64 SUBPASSES{ 1 };

		constexpr uint32 SCENE_INTERMEDIATE_EIGHTH_INDEX{ 0 };
		...
		constexpr StaticArray<const VkAttachmentReference, 1> colorAttachmentReferences
		{
			VkAttachmentReference{ SCENE_INTERMEDIATE_EIGHTH_INDEX, VK_IMAGE_LAYOUT_GENERAL }
		};
		...
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleThirdIteration)]._RenderPass = VulkanInterface::Instance->CreateRenderPass(renderPassParameters);

		//Create the framebuffer.
		VulkanFramebufferCreationParameters framebufferParameters;

		framebufferParameters._RenderPass = _VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleThirdIteration)]._RenderPass->Get();
		...
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleThirdIteration)]._FrameBuffers.Reserve(1);
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleThirdIteration)]._FrameBuffers.EmplaceFast(VulkanInterface::Instance->CreateFramebuffer(framebufferParameters));
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleThirdIteration)]._NumberOfAttachments = 1;
	...
	//Initialize the bloom downsample fifth iteration render pass.
	{
		constexpr uint64 SUBPASSES{ 1 };

		constexpr uint32 INTERMEDIATE_THIRTYSECOND_INDEX{ 0 };
		...
		{
			//Screen.
			VulkanUtilities::CreateAttachmentDescription(	static_cast<VulkanRenderTarget *const RESTRICT>(RenderingSystem::Instance->GetRenderTarget(RenderTarget::IntermediateThirtySecond))->GetFormat(),
															VK_ATTACHMENT_LOAD_OP_DONT_CARE,
															VK_ATTACHMENT_STORE_OP_STORE,
															VK_ATTACHMENT_LOAD_OP_DONT_CARE,
															VK_ATTACHMENT_STORE_OP_DONT_CARE,
		...
		constexpr StaticArray<const VkAttachmentReference, 1> colorAttachmentReferences
		{
			VkAttachmentReference{ INTERMEDIATE_THIRTYSECOND_INDEX, VK_IMAGE_LAYOUT_GENERAL }
		};
		...
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleFifthIteration)]._RenderPass = VulkanInterface::Instance->CreateRenderPass(renderPassParameters);

		//Create the framebuffer.
		VulkanFramebufferCreationParameters framebufferParameters;

		framebufferParameters._RenderPass = _VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleFifthIteration)]._RenderPass->Get();

		StaticArray<VkImageView, 1> attachments
		{
			static_cast<VulkanRenderTarget *const RESTRICT>(RenderingSystem::Instance->GetRenderTarget(RenderTarget::IntermediateThirtySecond))->GetImageView()
		};

		framebufferParameters._AttachmentCount = static_cast<uint32>(attachments.Size());
		framebufferParameters._Attachments = attachments.Data();
		framebufferParameters._Extent = { RenderingSystem::Instance->GetScaledResolution()._Width / 32, RenderingSystem::Instance->GetScaledResolution()._Height / 32 };

		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleFifthIteration)]._FrameBuffers.Reserve(1);
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleFifthIteration)]._FrameBuffers.EmplaceFast(VulkanInterface::Instance->CreateFramebuffer(framebufferParameters));
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleFifthIteration)]._NumberOfAttachments = 1;
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomDownsampleFifthIteration)]._ShouldClear = false;
	}
	...
	//Initialize the bloom upsample first iteration render pass.
	{
		...
		constexpr uint32 INTERMEDIATE_SIXTEENTH_INDEX{ 0 };
		...
		constexpr StaticArray<const VkAttachmentReference, 1> colorAttachmentReferences
		{
			VkAttachmentReference{ INTERMEDIATE_SIXTEENTH_INDEX, VK_IMAGE_LAYOUT_GENERAL }
		};
	...
	//Initialize the bloom upsample third iteration render pass.
	{
		...
		constexpr uint32 INTERMEDIATE_QUARTER_INDEX{ 0 };
		...
		constexpr StaticArray<const VkAttachmentReference, 1> colorAttachmentReferences
		{
			VkAttachmentReference{ INTERMEDIATE_QUARTER_INDEX, VK_IMAGE_LAYOUT_GENERAL }
		};
	...
	//Initialize the bloom upsample fourth iteration render pass.
	{
		...
		constexpr uint32 INTERMEDIATE_HALF_INDEX{ 0 };
		...
		constexpr StaticArray<const VkAttachmentReference, 1> colorAttachmentReferences
		{
			VkAttachmentReference{ INTERMEDIATE_HALF_INDEX, VK_IMAGE_LAYOUT_GENERAL }
		};
	...
	//Initialize the bloom upsample fifth iteration render pass.
	{
		constexpr uint64 SUBPASSES{ 1 };
		...
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomUpsampleFifthIteration)]._RenderPass = VulkanInterface::Instance->CreateRenderPass(renderPassParameters);

		//Create the framebuffer.
		VulkanFramebufferCreationParameters framebufferParameters;

		framebufferParameters._RenderPass = _VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomUpsampleFifthIteration)]._RenderPass->Get();
		...
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomUpsampleFifthIteration)]._FrameBuffers.Reserve(1);
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomUpsampleFifthIteration)]._FrameBuffers.EmplaceFast(VulkanInterface::Instance->CreateFramebuffer(framebufferParameters));
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomUpsampleFifthIteration)]._NumberOfAttachments = 1;
		_VulkanRenderPassMainStageData[UNDERLYING(RenderPassMainStage::BloomUpsampleFifthIteration)]._ShouldClear = false;
	}
