...
void CommandBuffer::Begin(const Pipeline *const RESTRICT pipeline) NOEXCEPT
{
	...
	const VulkanPipelineSubStageData *const RESTRICT pipelineSubStageData{ static_cast<const VulkanPipelineSubStageData *const RESTRICT>(static_cast<const GraphicsPipeline *const RESTRICT>(pipeline)->GetData()) };

	//Begin the command buffer.
	reinterpret_cast<VulkanCommandBuffer *const RESTRICT>(_CommandBufferData)->BeginSecondary(VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT | VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT, pipelineSubStageData->_RenderPass, VulkanTranslationUtilities::GetSubStageIndex(static_cast<const GraphicsPipeline *const RESTRICT>(pipeline)->GetMainStage(), static_cast<const GraphicsPipeline *const RESTRICT>(pipeline)->GetSubStage()), static_cast<const GraphicsPipeline *const RESTRICT>(pipeline)->GetRenderTargets().Empty() ? pipelineSubStageData->_Framebuffers[0] : static_cast<const GraphicsPipeline *const RESTRICT>(pipeline)->GetRenderTargets()[0] == RenderTarget::Screen ? pipelineSubStageData->_Framebuffers[RenderingSystem::Instance->GetCurrentFramebufferIndex()] : pipelineSubStageData->_Framebuffers[0]);
