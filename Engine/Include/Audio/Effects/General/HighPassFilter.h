...
class HighPassFilter final : public AudioEffect
{

public:

	//Constants.
	constexpr static float32 DEFAULT_FREQUENCY{ 20.0f };
	...
	*	Default constructor.
	*/
	FORCE_INLINE HighPassFilter(const float32 frequency = DEFAULT_FREQUENCY, const float32 quality = DEFAULT_QUALITY, const uint32 order = DEFAULT_ORDER) NOEXCEPT
		:
...
private:
	...
	*/
	...
	public:
		...
		//The previous input samples.
		StaticArray<float32, 2> _PreviousInputSamples{ 0.0f };
	...
	*/
	FORCE_INLINE void Reset() NOEXCEPT
	{
		...
		for (uint32 i{ 0 }; i < number_of_second_order_filters; ++i)
		{
			...
			{
				biquad.InitializeHighPass(_Frequency, _Quality, _SampleRate);
			}
		}
	...
	*/
	FORCE_INLINE NO_DISCARD float32 ProcessFirstOrderFilter(const uint8 channel_index, const float32 input_sample, FirstOrderFilter *const RESTRICT first_order_filter) NOEXCEPT
	{
		...
		{
			(first_order_filter->_PreviousOutputSamples[channel_index] + input_sample - first_order_filter->_PreviousInputSamples[channel_index]) * first_order_filter->_Alpha
		};

		first_order_filter->_PreviousInputSamples[channel_index] = input_sample;
