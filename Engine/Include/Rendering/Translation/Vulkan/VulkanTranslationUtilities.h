	...
	*/
	static VkFormat GetVulkanFormat(const VertexInputAttributeDescription::Format format) NOEXCEPT
	{
		switch (format)
		{
			...
			case VertexInputAttributeDescription::Format::X32Y32Z32W32UnsignedInt: return VK_FORMAT_R32G32B32A32_UINT;
...
#define MAPPING(FLAG, BIT) if (TEST_BIT(pipeline_stage_flags, FLAG)) flags |= BIT;
		...
		MAPPING(PipelineStageFlags::EARLY_FRAGMENT_TESTS, VkPipelineStageFlagBits::VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT);
		...
		MAPPING(PipelineStageFlags::TRANSFER, VkPipelineStageFlagBits::VK_PIPELINE_STAGE_TRANSFER_BIT);
