...
CATALYST_SHADER_NAMESPACE_BEGIN(CatalystLighting)
	...
	CATALYST_SHADER_FUNCTION_RETURN_NINE_ARGUMENTS(	vec3,
													CalculateDirectBidirectionalReflectanceDistribution,
	...
	CATALYST_SHADER_FUNCTION_RETURN_NINE_ARGUMENTS(	vec3,
													CalculateDirectSpecularComponent,
													vec3 outgoing_direction,
													vec3 albedo,
													vec3 normal,
													float roughness,
	...
	CATALYST_SHADER_FUNCTION_RETURN_NINE_ARGUMENTS(	vec3,
													CalculateDirectNormalDistribution,
													vec3 outgoing_direction,
													vec3 albedo,
													vec3 normal,
													float roughness,
	...
	CATALYST_SHADER_FUNCTION_RETURN_NINE_ARGUMENTS(	vec3,
													CalculateDirectDiffuseComponent,
													vec3 outgoing_direction,
													vec3 albedo,
													vec3 normal,
													float roughness,
	...
	*/
													...
													vec3 irradiance)
	{
		...
		vec3 bidirectional_reflectance_distribution = CalculateDirectBidirectionalReflectanceDistribution(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
	...
	*/
													...
													vec3 irradiance)
	{
		...
		vec3 specular_component = CalculateDirectSpecularComponent(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);

		//Calculate the diffse component.
		vec3 diffuse_component = CalculateDirectDiffuseComponent(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);

		//Calculate the specular weight	
		vec3 specular_weight = CalculateDirectSpecularWeight(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
	...
	*/
													...
													vec3 irradiance)
	{
		...
		vec3 normal_distribution = CalculateDirectNormalDistribution(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
		vec3 geometry = CalculateDirectGeometry(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
		vec3 fresnel = CalculateDirectFresnel(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
	...
	*/
													...
													vec3 irradiance)
	{
		...
		return CalculateDirectFresnel(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
	}
	...
	*/
	...
	{
		//Calculate the bidirectional reflectance distribution.
		vec3 bidirectional_reflectance_distribution = CalculateIndirectBidirectionalReflectanceDistribution(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
	...
	*/
	...
	{
		//Calculate the specular component.
		vec3 specular_component = CalculateIndirectSpecularComponent(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);

		//Calculate the diffse component.
		vec3 diffuse_component = CalculateIndirectDiffuseComponent(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);

		//Calculate the specular weight	
		vec3 specular_weight = CalculateIndirectSpecularWeight(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
	...
	*/
													...
													vec3 irradiance)
	{
		...
		//Calculate the numerator.
		vec3 normal_distribution = CalculateIndirectNormalDistribution(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
		vec3 geometry = CalculateIndirectGeometry(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
		vec3 fresnel = CalculateIndirectFresnel(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
	...
	*/
	...
	{
		//The fresnel is directly related to the specular weight, so use that.
		return CalculateIndirectFresnel(outgoing_direction, albedo, normal, roughness, metallic, ambient_occlusion, thickness, irradiance_direction, irradiance);
	}
