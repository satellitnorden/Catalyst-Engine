...
namespace TerrainGeneralUtilities
{
	...
	*/
	FORCE_INLINE static void GenerateTerrainPlane(const TerrainProperties& properties, const Vector3<float> &patch_world_position, const float patch_size, DynamicArray<TerrainVertex> *const RESTRICT vertices, DynamicArray<uint32> *const RESTRICT indices) NOEXCEPT
	{
		...
		for (uint32 X{ 0 }; X < TerrainConstants::TERRAIN_PATCH_RESOLUTION; ++X)
		{
			for (uint32 Y{ 0 }; Y < TerrainConstants::TERRAIN_PATCH_RESOLUTION; ++Y)
			{
				//Construct the vertex.
				TerrainVertex vertex;

				//Calculate the world position.
				const float coordinate_x{ static_cast<float>(X) / static_cast<float>(TerrainConstants::TERRAIN_HEIGHT_TEXTURE_RESOLUTION - 1) };
				const float coordinate_y{ static_cast<float>(Y) / static_cast<float>(TerrainConstants::TERRAIN_HEIGHT_TEXTURE_RESOLUTION - 1) };

				Vector3<float> world_position{	patch_world_position._X + ((-1.0f + (2.0f * coordinate_x)) * (patch_size * 0.5f)),
												0.0f,
												patch_world_position._Z + ((-1.0f + (2.0f * coordinate_y)) * (patch_size * 0.5f)) };

				vertex._Position._X = world_position._X;
				vertex._Position._Z = world_position._Z;

				//Generate the height.
				properties._HeightFunction(properties, world_position, &vertex._Position._Y, nullptr);

				vertices->EmplaceFast(vertex);

				//Construct the indices.
				if (X != TerrainConstants::TERRAIN_PATCH_RESOLUTION - 1 && Y != TerrainConstants::TERRAIN_PATCH_RESOLUTION - 1)
				{
					indices->EmplaceFast((X * TerrainConstants::TERRAIN_PATCH_RESOLUTION) + Y);
					indices->EmplaceFast((X * TerrainConstants::TERRAIN_PATCH_RESOLUTION) + Y + 1);
					indices->EmplaceFast(((X + 1) * TerrainConstants::TERRAIN_PATCH_RESOLUTION) + Y);

					indices->EmplaceFast((X * TerrainConstants::TERRAIN_PATCH_RESOLUTION) + Y + 1);
					indices->EmplaceFast(((X + 1) * TerrainConstants::TERRAIN_PATCH_RESOLUTION) + Y + 1);
					indices->EmplaceFast(((X + 1) * TerrainConstants::TERRAIN_PATCH_RESOLUTION) + Y);
				}
			}
		}
