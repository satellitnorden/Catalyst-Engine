...
public:
	...
	*/
	FORCE_INLINE static NO_DISCARD Vector3<float> GenerateDerivaties(const Vector2<float> &coordinates, const float seed = 0.0f) NOEXCEPT
	{
		...
		{
			Gradient(SimplexNoiseConstants::PERMUTATIONS[ii + SimplexNoiseConstants::PERMUTATIONS[jj]], &gx0, &gy0);
			t20 = t0 * t0;
			t40 = t20 * t20;
		...
		{
			Gradient(SimplexNoiseConstants::PERMUTATIONS[ii + i1 + SimplexNoiseConstants::PERMUTATIONS[jj + j1]], &gx1, &gy1);
			t21 = t1 * t1;
			t41 = t21 * t21;
		...
		{
			Gradient(SimplexNoiseConstants::PERMUTATIONS[ii + 1 + SimplexNoiseConstants::PERMUTATIONS[jj + 1]], &gx2, &gy2);
			t22 = t2 * t2;
			t42 = t22 * t22;
...
private:
	...
	*/
	FORCE_INLINE static void Gradient(const int32 hash, float *const RESTRICT gx, float *const RESTRICT gy) NOEXCEPT
	{
		//Define constants.
