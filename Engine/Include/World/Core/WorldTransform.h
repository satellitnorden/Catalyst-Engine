...
public:
	...
	*/
	FORCE_INLINE constexpr WorldTransform(	const Vector3<int32> &initial_cell,
	...
	*/
	FORCE_INLINE constexpr WorldTransform(	const Vector3<float32> &initial_local_position,
											const Vector3<float32> &initial_rotation,
											const float32 initial_scale) NOEXCEPT
		:
		_Cell(0, 0, 0),
	...
	*/
	FORCE_INLINE NO_DISCARD const Vector3<float32> &GetAbsolutePosition() const NOEXCEPT
	{	
	...
	*/
	FORCE_INLINE NO_DISCARD const Vector3<float32> &GetRelativePosition(const Vector3<int32> &cell) const NOEXCEPT
	{	
		const float32 world_grid_size{ WorldSystem::Instance->GetWorldGridSize() };

		const Vector3<int32> delta{ _Cell - cell };

		return _LocalPosition + Vector3<float32>(static_cast<float32>(delta._X), static_cast<float32>(delta._Y), static_cast<float32>(delta._Z)) * world_grid_size * world_grid_size;
	}
