...
namespace nam
{
namespace parametric_slimmable_wavenet
{
...
#else
void ParametricSlimmableWavenet::_pending_clear_release()
{
  _pending_staged.store({}, std::memory_order_release);
}

std::shared_ptr<ParametricSlimmableWavenet::StagedSlimModel> ParametricSlimmableWavenet::_pending_load_acquire() const
{
  return _pending_staged.load(std::memory_order_acquire);
}

void ParametricSlimmableWavenet::_pending_store_release(std::shared_ptr<StagedSlimModel> p)
{
  _pending_staged.store(std::move(p), std::memory_order_release);
}

std::shared_ptr<ParametricSlimmableWavenet::StagedSlimModel> ParametricSlimmableWavenet::_pending_exchange_take_acq_rel()
{
...
ParametricSlimmableWavenet::ParametricSlimmableWavenet(std::vector<wavenet::LayerArrayParams> original_params,
...
std::unique_ptr<DSP> ParametricSlimmableWavenet::_create_wavenet_for_channels(const std::vector<int>& target_channels)
{
  ...
  return std::make_unique<parametric_wavenet::ParametricWaveNet>(_in_channels, *params_ptr, _head_scale, _with_head, std::nullopt,
                                            std::move(weights), std::move(condition_dsp), sampleRate);
}

void ParametricSlimmableWavenet::_rebuild_model(const std::vector<int>& target_channels)
{
...
void ParametricSlimmableWavenet::_stage_rebuild_model(const std::vector<int>& target_channels)
{
  if (target_channels == _current_channels && _active_model)
  {
    _pending_clear_release();
...
void ParametricSlimmableWavenet::process(NAM_SAMPLE** input, NAM_SAMPLE** output, const int num_frames)
{
...
void ParametricSlimmableWavenet::prewarm()
{
  if (_active_model)
...
void ParametricSlimmableWavenet::Reset(const double sampleRate, const int maxBufferSize)
{
...
void ParametricSlimmableWavenet::SetSlimmableSize(const double val)
{
...
std::unique_ptr<DSP> SlimmableWavenetConfig::create(std::vector<float> weights, double sampleRate)
{
  ...
  auto wc = parametric_wavenet::parse_config_json(model_json, sampleRate);
  ...
  return std::make_unique<ParametricSlimmableWavenet>(std::move(wc.layer_array_params), std::move(per_array_allowed),
