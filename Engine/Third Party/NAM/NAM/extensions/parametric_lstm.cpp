...
#include "..\registry.h"
#include "parametric_lstm.h"

nam::parametric_lstm::LSTMCell::LSTMCell(const int input_size, const int hidden_size, std::vector<float>::iterator& weights)
{
  // Resize arrays
...
void nam::parametric_lstm::LSTMCell::process_(const Eigen::VectorXf& x)
{
...
nam::parametric_lstm::LSTM::LSTM(const int num_layers, const int input_size, const int hidden_size, std::vector<float>& weights,
...
void nam::parametric_lstm::LSTM::process(NAM_SAMPLE* input, NAM_SAMPLE* output, const int num_frames)
{
  for (int i = 0; i < num_frames; i++)
    output[i] = this->_process_sample(&input[i * this->_input_size]);
}

int nam::parametric_lstm::LSTM::PrewarmSamples()
{
#if 0
...
#else
    /*
    *   The default prewarm is only set up for non-parametric models,
    *   so just completely disable for now until I can figure out a better way to do it... (:
    */
    return 0;
#endif
}

float nam::parametric_lstm::LSTM::_process_sample(const float *x)
{
  if (this->_layers.size() == 0)
    return x[0];

  for (int i = 0; i < this->_input_size; ++i)
  {
      this->_input(i) = x[i];
  }
...
void nam::parametric_lstm::RegisterFactory()
{
    static bool ONCE = false;

    if (!ONCE)
    {
        nam::factory::FactoryRegistry::instance().registerFactory("ParametricLSTM", nam::parametric_lstm::Factory);
...
std::unique_ptr<nam::DSP> nam::parametric_lstm::Factory(const nlohmann::json& config, std::vector<float>& weights,
                                             const double expectedSampleRate)
{
  const int num_layers = config["num_layers"];
  const int input_size = config["input_size"];
  const int hidden_size = config["hidden_size"];
  return std::make_unique<nam::parametric_lstm::LSTM>(num_layers, input_size, hidden_size, weights, expectedSampleRate);
}
