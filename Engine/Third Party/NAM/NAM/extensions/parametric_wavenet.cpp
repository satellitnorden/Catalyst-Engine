...
nam::parametric_wavenet::ParametricWaveNet::ParametricWaveNet(const int in_channels,
    ...
    const float head_scale, const bool with_head, std::optional<wavenet::HeadParams> head_params,
    ...
    , _head_scale(head_scale)
{
    ...
    if (with_head)
    {
        ...
        this->_post_stack_head = std::make_unique<wavenet::detail::Head>(*head_params);
    }
...
void nam::parametric_wavenet::ParametricWaveNet::SetPrewarmOnReset(const bool prewarmOnReset)
{
...
void nam::parametric_wavenet::ParametricWaveNet::prewarm()
{
    /* Actually, do nothing. (:
...
bool nam::parametric_wavenet::ParametricWaveNet::HasCachedPrewarmState() const
{
    if (_condition_dsp != nullptr)
        return false;
    if (!std::all_of(_layer_arrays.begin(), _layer_arrays.end(),
        [](const wavenet::detail::LayerArray& layer_array) { return layer_array.HasCachedPrewarmState(); }))
        return false;
    return _post_stack_head == nullptr || _post_stack_head->HasCachedPrewarmState();
}

void nam::parametric_wavenet::ParametricWaveNet::PrewarmFromCache()
{
...
void nam::parametric_wavenet::ParametricWaveNet::CacheStateAsPrewarmed()
{
    if (_condition_dsp != nullptr)
        return;
    for (auto& layer_array : _layer_arrays)
    ...
    const double expectedSampleRate)
{
    ...
    for (size_t i = 0; i < config["layers"].size(); i++)
    {
        ...
        std::vector<wavenet::GatingMode> gating_modes;
        std::vector<activations::ActivationConfig> secondary_activation_configs;

        auto parse_gating_mode_str = [](const std::string& gating_mode_str) -> wavenet::GatingMode {
            if (gating_mode_str == "gated")
                return wavenet::GatingMode::GATED;
            else if (gating_mode_str == "blended")
                return wavenet::GatingMode::BLENDED;
            else if (gating_mode_str == "none")
                return wavenet::GatingMode::NONE;
            else
        ...
        if (layer_config.find("gating_mode") != layer_config.end())
        {
            if (layer_config["gating_mode"].is_array())
            {
                for (const auto& gating_mode_json : layer_config["gating_mode"])
                {
                    ...
                    wavenet::GatingMode mode = parse_gating_mode_str(gating_mode_str);
                    gating_modes.push_back(mode);

                    // Parse corresponding secondary activation if gating is enabled
                    if (mode != wavenet::GatingMode::NONE)
                    {
            ...
            else
            {
                ...
                wavenet::GatingMode gating_mode = parse_gating_mode_str(gating_mode_str);
                gating_modes.resize(num_layers, gating_mode);

                activations::ActivationConfig secondary_activation_config;
                if (gating_mode != wavenet::GatingMode::NONE)
                {
                    if (layer_config.find("secondary_activation") != layer_config.end())
                    {
        ...
        else if (layer_config.find("gated") != layer_config.end())
        {
            ...
            wavenet::GatingMode gating_mode = gated ? wavenet::GatingMode::GATED : wavenet::GatingMode::NONE;
            gating_modes.resize(num_layers, gating_mode);
        ...
        else
        {
            ...
            gating_modes.resize(num_layers, wavenet::GatingMode::NONE);
            secondary_activation_configs.resize(num_layers, activations::ActivationConfig{});
        }
...
namespace
{
    static nam::ConfigParserHelper _register_ParametricWaveNet("ParametricWaveNet", nam::parametric_wavenet::create_config);
}
*/