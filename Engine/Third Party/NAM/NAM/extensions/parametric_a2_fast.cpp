...
public:
  ...
  A2FastModel(std::vector<float> weights, double expected_sample_rate, const unsigned condition_size);
...
// -----------------------------------------------------------------------------
template <int Channels>
A2FastModel<Channels>::A2FastModel(std::vector<float> weights, double expected_sample_rate, const unsigned condition_size)
: DSP(/*in_channels=*/1, /*out_channels=*/1, expected_sample_rate),
_condition_size(condition_size)
{
...
void A2FastModel<Channels>::process(NAM_SAMPLE** input, NAM_SAMPLE** output, int num_frames)
{
  ...
  for (int f = 0; f < num_frames; f++)
  {
    const float x = static_cast<float>(input[0][f]);
    cond[f] = static_cast<float>(input[1][f]);
...
{
  unsigned channels = 0;
  unsigned condition_size = 1;

  std::unique_ptr<DSP> create(std::vector<float> weights, double sampleRate) override
  {
    if (channels == 3)
      return std::make_unique<A2FastModel<3>>(std::move(weights), sampleRate, condition_size);
    if (channels == 8)
      return std::make_unique<A2FastModel<8>>(std::move(weights), sampleRate, condition_size);
...
// Public API
// -----------------------------------------------------------------------------
bool is_a2_shape(const nlohmann::json& config, unsigned *channels, unsigned *condition_size)
{
  ...
  const int cs = la.value("condition_size", 0);
