...
void CatalystShaderMain()
{
  ...
  float distance_based_opacity = CalculateDistanceBasedOpacity(fragment_distance_squared, cutoff_distance_squared);

  //Retrieve the random opacity.
  if (distance_based_opacity < 1.0f)
  {
      //Discard conditionally based on the opacity.
      if (ShouldClip(uint(gl_FragCoord.x), uint(gl_FragCoord.y), distance_based_opacity))
      {
          discard;
      }
  }
