...
vec3 CalculateWindDisplacement(vec3 instance_position, vec3 vertex_position, vec3 vertex_normal, float time, float large_scale_wind_displacement_factor, float medium_scale_wind_displacement_factor, float small_scale_wind_displacement_factor)
{
	...
	float time_factor = time * (1.0f + wind_speed_multiplier);
    ...
    //Medium scale motion.
    float medium_scale_X = windDirection.x * (sin((instance_position.x + instance_position.y + instance_position.z + time_factor) * EULERS_NUMBER) + 0.5f) * 0.5f;
    float medium_scale_Z = windDirection.z * (cos((instance_position.x + instance_position.y + instance_position.z + time_factor) * PHI) + 0.5f) * 0.5f;

    medium_scale_X *= medium_scale_wind_displacement_factor;
    medium_scale_Z *= medium_scale_wind_displacement_factor;

    //Small scale motion.
    float small_scale_X = windDirection.x * sin((vertex_position.x + vertex_position.y + vertex_position.z + time_factor) * DOUBLE_PI) * 0.25f * vertex_normal.x;
    float small_scale_Y =                   cos((vertex_position.x + vertex_position.y + vertex_position.z + time_factor) * PI) * 0.25f * vertex_normal.y;
    float small_scale_Z = windDirection.z * sin((vertex_position.x + vertex_position.y + vertex_position.z + time_factor) * DOUBLE_PI) * 0.25f * vertex_normal.z;
