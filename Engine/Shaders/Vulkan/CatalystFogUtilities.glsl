...
#define FOG_DIRECTIONAL_LIGHT_SCALE (0.0625f)
...
vec3 CalculateFogColor(vec3 viewDirection)
{
	...
	vec3 baseColor = mix(FOG_BASE_COLOR, directionalLightColor, clamp(dot(viewDirection, -directionalLightDirection) + directionalLightInfluence, 0.0f, 1.0f));
