...
vec3 Irradiance(vec3 origin, vec3 direction)
{
	...
	{
		//Calculate the screen space coordinates for the current position.
		vec4 screen_space_coordinate = viewMatrix * vec4(current_position, 1.0f);
		float inverse_denominator = 1.0f / screen_space_coordinate.w;
		screen_space_coordinate.xy = screen_space_coordinate.xy * inverse_denominator * 0.5f + 0.5f;

		//Break if the screen_space coordinate is outside the screen.
		if (!ValidCoordinate(screen_space_coordinate.xy))
		{
			break;
		}

		//Calculate the expected depth.
		float expected_depth = screen_space_coordinate.z * inverse_denominator;
...
void SampleRandomDirectionAndLength(vec3 normal, out vec3 direction, out float length)
{
	...
	vec4 random_direction_and_length_sample = RANDOM_SAMPLES[(int(gl_FragCoord.x) + int(gl_FragCoord.y) + int(random_sample.w * 64.0f)) & 63];
