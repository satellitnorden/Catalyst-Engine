...
void CatalystShaderMain()
{
	...
	vec3 current_indirect_lighting = texture(sampler2D(RENDER_TARGETS[SOURCE_RENDER_TARGET_INDEX], GLOBAL_SAMPLERS[GLOBAL_SAMPLER_FILTER_NEAREST_MIPMAP_MODE_NEAREST_ADDRESS_MODE_CLAMP_TO_EDGE_INDEX]), fragment_texture_coordinate).rgb;
	...
	vec3 minimum = current_indirect_lighting;
	...
	for (int y = -1; y <= 1; y += 1)
	{
		for (int x = -1; x <= 1; x += 1)
		{
			...
			*/
			...
			minimum = mix(minimum, min(minimum, sample_indirect_lighting), sample_weight);
		}
	}
