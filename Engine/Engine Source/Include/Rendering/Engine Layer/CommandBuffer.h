...
public:
	...
	*/
	virtual void BindVertexBuffers(const RenderPass *const RESTRICT renderPass, const uint32 numberOfVertexBuffers, const ConstantBufferHandle *const RESTRICT vertexBuffers, const uint64 *const RESTRICT offsets) NOEXCEPT = 0;
	...
	*	Waits for events.
	*/
	virtual void WaitForEvents(const RenderPass *const RESTRICT renderPass, const uint32 eventCount, const EventHandle *const RESTRICT events) NOEXCEPT = 0;
