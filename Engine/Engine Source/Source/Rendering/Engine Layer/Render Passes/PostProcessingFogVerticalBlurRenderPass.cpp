//Header file.
#include <Rendering/Engine Layer/Render Passes/PostProcessingFogVerticalBlurRenderPass.h>
...
DEFINE_SINGLETON(PostProcessingFogVerticalBlurRenderPass);

/*
*	Default constructor.
*/
PostProcessingFogVerticalBlurRenderPass::PostProcessingFogVerticalBlurRenderPass() NOEXCEPT
{
	//Set the initialization function.
	SetInitializationFunction([](void *const RESTRICT)
	{
		PostProcessingFogVerticalBlurRenderPass::Instance->InitializeInternal();
	});
}

/*
*	Initializes the post processing fog vertical blur render pass.
*/
void PostProcessingFogVerticalBlurRenderPass::InitializeInternal() NOEXCEPT
{
	//Set the stage.
	SetStage(RenderPassStage::PostProcessingFogVerticalBlur);
	...
	AddRenderTarget(RenderTarget::FogBlur);
	...
	{
		PostProcessingFogVerticalBlurRenderPass::Instance->RenderInternal();
	});
...
*/
void PostProcessingFogVerticalBlurRenderPass::RenderInternal() NOEXCEPT
{
	...
	StaticArray<RenderDataTableHandle, 2> descriptorSets
	{
		...
		RenderingSystem::Instance->GetRenderDataTable(RenderDataTable::PostProcessingFogVerticalBlur)
	};
