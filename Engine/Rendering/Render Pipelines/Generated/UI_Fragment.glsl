...
//Constants.
#define RENDER_COMMAND_MODE_NONE (0)
#define RENDER_COMMAND_MODE_COLOR (1)
#define RENDER_COMMAND_MODE_TEXTURE (2)
#define RENDER_COMMAND_MODE_TEXT (3)
//UI render command struct definition.
...
layout (std430, set = 1, binding = 0) buffer UI
{
	layout (offset = 0) RenderCommand[] RENDER_COMMANDS;
};
...
void main()
{
    ...
    switch (RENDER_COMMANDS[InInstanceIndex]._Mode)
    {
        ...
        case RENDER_COMMAND_MODE_TEXTURE:
        {
            color = texture(sampler2D(TEXTURES[RENDER_COMMANDS[InInstanceIndex]._ColorOrTexture], SAMPLER), InTextureCoordinate);
            float signed_distance = RoundedRectangleSignedDistance
            (
                ClipToPixel(RENDER_COMMANDS[InInstanceIndex]._Positions[0].xy),
                ClipToPixel(RENDER_COMMANDS[InInstanceIndex]._Positions[2].xy),
