...
{
	layout (offset = 0) vec4 POSITION_ROTATION;
	layout (offset = 16) vec4 COLOR;
	layout (offset = 32) vec2 SIZE;
};

layout (location = 0) out vec2 OutPreRotationPosition;
...
void main()
{
    ...
    (
        mix(-0.5f, 0.5f, float(gl_VertexIndex / 2)) * SIZE.x,
        mix(-0.5f, 0.5f, float(gl_VertexIndex & 1)) * SIZE.y
    );
