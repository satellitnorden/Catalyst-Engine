...
public:
	...
	public:
		...
		//The mask texture file path for the impostor.
		const char *RESTRICT _ImpostorMaskFile;

		//The number of mipmap levels for the impostor.
		uint8 _ImpostorMipmapLevels;

		//The albedo file path for the impostor.
		const char *RESTRICT _ImpostorAlbedoFile;
	...
	*/
	static void CreateTreeVegetationMaterial(const TreeVegetationMaterialCreationParameters &parameters) noexcept
	{
		...
		//Impostor.
		{
			//Write the number of mipmap levels that should be generated for the impostor mask texture to the file.
			file.Write(&parameters._ImpostorMaskMipmapLevels, sizeof(uint8));

			//Load the impostor mask texture.
			int32 width, height, numberOfChannels;
			byte *RESTRICT data{ stbi_load(parameters._ImpostorMaskFile, &width, &height, &numberOfChannels, STBI_rgb_alpha) };

			uint32 uWidth{ static_cast<uint32>(width) };
			uint32 uHeight{ static_cast<uint32>(height) };

			//Write the width and height of the impostor mask texture into the file.
			file.Write(&uWidth, sizeof(uint32));
			file.Write(&uHeight, sizeof(uint32));

			//Write the impostor mask texture to the file.
			for (uint8 i = 0; i < parameters._ImpostorMaskMipmapLevels; ++i)
			{
				const uint64 textureSize{ (uWidth >> i) * (uHeight >> i) * 4 };
			...
			//Write the number of impostor mipmap levels  to the file.
			file.Write(&parameters._ImpostorMipmapLevels, sizeof(uint8));

			//Load the impostor albedo texture.
			data = stbi_load(parameters._ImpostorAlbedoFile, &width, &height, &numberOfChannels, STBI_rgb_alpha);
			...
			//Write the impostor albedo texture to the file.
			for (uint8 i = 0; i < parameters._ImpostorMipmapLevels; ++i)
			{
				const uint64 textureSize{ (uWidth >> i) * (uHeight >> i) * 4 };
